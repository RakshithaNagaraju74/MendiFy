...
const HealthAIPage: React.FC = () => {
  ...
  useEffect(() => {
    const initializeSession = async () => {
      ...
      console.log('🛠 Local Storage sessionId:', storedId);
      ...
      if (!storedId) {
        try {
          ...
          if (storedId) {
            ...
            console.log('💾 Stored sessionId in localStorage:', storedId);
          }
        } catch (err) {
      ...
      console.log('✅ Session ID set to state:', storedId);
    };
  ...
  useEffect(() => {
    if (sessionId) {
      const defaultMessage: Message = {
        sender: 'ai',
        content: "Hello! How can I assist with your health today? 🩺",
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await axios.post(`/api/health-ai/chat`, {
        message: input,
        sessionId, // 👈 if your API expects sessionId
      });
  ...
  const addEmojiReaction = (index: number, emoji: string) => {
    console.log(`👍 Adding emoji "${emoji}" to message at index ${index}`);
    setMessages((prev) =>
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-teal-100 via-green-100 to-yellow-100 p-4 md:p-6 relative">
      ...
      <div className="flex-1 overflow-y-auto space-y-6 px-2 py-4 md:p-6 bg-white rounded-3xl shadow-2xl border border-gray-200 backdrop-blur-md">
        {messages.map((msg, index) => (
          ...
          >
            <div className="min-w-[40px] min-h-[40px] rounded-full bg-gradient-to-tr from-teal-200 to-green-200 flex items-center justify-center text-xl shadow">
              {msg.sender === 'user' ? '👤' : '🤖'}
            </div>
            <div
      ...
      <div className="flex space-x-4 items-center bg-white px-4 py-3 mt-4 rounded-full shadow-2xl border border-gray-300 backdrop-blur-lg sticky bottom-4">
        <input
          ...
          className="flex-1 px-4 py-2 rounded-full border border-gray-300 focus:ring-2 focus:ring-teal-400 outline-none transition"
        />
        <button
          onClick={handleSend}
          disabled={loading}
          className="bg-teal-500 hover:bg-teal-600 text-white font-bold py-2 px-5 rounded-full shadow-md disabled:opacity-50 transition-all duration-200"
        >
          Send
        </button>
      </div>
